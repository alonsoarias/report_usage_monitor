...
$string['messagehtml2'] = '<p>The platform <a href="{$a->siteurl}" target="_blank" ><strong>\'{$a->sitename}\'</strong></a> has exceeded 90% of the assigned disk space</p>
<p>Assigned disk space: {$a->quotadisk} </p>
<p>Used disk space: <strong>{$a->diskusage}</strong></p>
<p>Disk usage percentage: <strong>{$a->percentage}%</strong></p>
<p>Database size: <strong>{$a->databasesize}</strong></p>
<p>Moodledata size: <strong>{$a->moodledata}</strong></p>
<p>Number of backups per course: <strong>{$a->backupcount}</strong></p>
<p>Set threshold for maximum daily users: <strong>{$a->threshold}</strong> users</p>
<strong>Monitor URL: </strong> {$a->referer}
<br>
<hr>
<i><p>This message has been automatically generated by the "Usage Report" from <a href="https://ingeweb.co/" target="_blank" ><strong>ingeweb.co</strong></a></p><i>';
